...
`document.documentElement`属性返回当前文档的根元素节点（root）。它通常是`document`节点的第二个子节点，紧跟在`document.doctype`节点后面。HTML网页的该属性，一般是`<embed>`节点。
...
`document.cookie`属性用来操作浏览器 Cookie，详见后续Cookie部分
...
##### 3.21 document.getSelection()
 ...
 这个方法指向`window.getSelection()`，参见`window`对象 