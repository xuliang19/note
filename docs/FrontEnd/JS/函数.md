...
JavaScript 有三种声明函数的方法，第一二种经常使用，第三种基本不用
...
这种写法将一个**匿名函数赋值给变量**。这时，**这个匿名函数又称函数表达式**（Function Expression），因为**赋值语句的等号右侧只能放表达式**。
...
如果同一个函数被多次声明，**后面的声明就会覆盖前面的声明**（如果采用`function`和赋值语句声明一个函数，那么总是赋值语句的生效）
...
函数体内部的`return`语句，表示返回。JavaScript 引擎遇到`return`语句，就直接返回`return`后面的那个表达式的值，**后面即使还有语句，也不会得到执行**。`return`语句不是必需的，如果没有的话，该函数就不返回任何值，或者说返回`undefined`。
...
JavaScript  语言**将函数看作一种值**，与其它值（数值、字符串、布尔值等等）地位相同。凡是可以使用值的地方，就能使用函数。比如，可以把函数赋值给变量和对象的属性，也可以当作参数传入其他函数，或者作为函数的结果返回。函数只是一个可以执行的值，此外并无特殊之处。
...
上面代码定义了空函数`f`，它的`length`属性就是定义时的参数个数。不管调用时输入了多少个参数，`length`属性始终等于2（因为`length`的目标是`argument`对象）

`length`属性提供了一种机制，判断定义时和调用时参数的差异，以便实现面向对象编程的“方法重载”（overload）（而函数重载的意思是同一个名字的函数，可以根据参数的不同有多个定义，而JS中没有，同一个函数名的函数只能有一个，后面的会覆盖前面的，不会像Java里面一样）
...
作用域（scope）指的是变量存在的范围。在 ES5 的规范中，JavaScript 只有两种作用域：一种是全局作用域，变量在整个程序中一直存在，所有地方都可以读取；另一种是函数作用域，变量只在函数内部存在。**ES6 又新增了块级作用域，ES6里面再写**
...
注意，如果函数内部修改的，不是参数对象的某个属性，**而是替换掉整个参数，这时不会影响到原始值**（这就相当于把内部指针指向另一个`object`了）
...
**通过`arguments`对象的`length`属性，可以判断函数调用时到底带几个参数**（而`f.length`则是判断函数定义时候的参数个数）
...
inc() // 6
inc() // 7
//如果这里不是闭包的话，每次调用都是一样的结果，因为变量执行完之后就被回收了
```
...
注意，上面两种写法**最后的分号都是必须的**。如果省略分号，遇到连着两个 IIFE，可能就会报错。
...
?> 可以在立即调用表达式中传入参数`(function(k){ /* code */ })(i);`（来自闭包）
