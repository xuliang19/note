...
对象的**所有键名都是字符串**（ES6 又引入了 Symbol 值也可以作为键名），所以加不加引号都可以。上面的代码也可以写成下面这样。
...
上面的代码中，当`x`的值发生变化后，`y`的值并不变，这就表示`y`和`x`并不是指向同一个内存地址（具体可查看引用类型）
...
注意，**数值键名不能使用点运算符**（因为会被当成小数点，任何情况为了防止数字`.`被当做小数点的办法是在产生歧义的地方加上括号`()`，不一定有效），只能使用方括号运算符。
...
?> 注意：对象没有`length`属性，获取长度可用`keys`，再用`length`，用着个可以区别对象和数组（类数组）
...
**`in`运算符**（注意：这是个运算符哦，`hasOwnproperty`是一个方法）用于检查对象是否包含某个属性（注意，**检查的是键名**，不是键值），如果包含就返回`true`，否则返回`false`。它的左边是一个字符串，表示属性名，右边是一个对象。
...
举例来说，对象都继承了`toString`属性，但是`for...in`循环不会遍历到这个属性（因为默认不可遍历）
