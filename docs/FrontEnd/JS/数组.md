...
值得注意的是，由于数组本质上是一种对象，所以可以为**数组添加属性**，但是这不影响`length`属性的值。
...
上面代码将数组的键分别设为字符串和小数，结果都不影响`length`属性。因为，**`length`属性的值就是等于最大的数字键加1**，而这个**数组没有整数键**，所以`length`属性保持为`0`（数组显示时不会显示键名为非整数的属性，所以上述代码中的属性虽然可以读取，但是不会显示在数组中）
...
**如果一个对象的所有键名都是正整数或零，并且有`length`属性**（要写在属性里面，导致它的`length`为定值，而数组的是继承的，具有动态性），那么这个对象就很像数组，语法上称为“类似数组的对象”（array-like object）。
