...
19. <p style="color:red">为什么调用时候有`setTimeout("x()",1000)`和`setTimeout(x,1000)`？
...
21. 通过用 `obj.style.opacity`来实时获取对象状态，然后用语句进行判断
...
22. 编程的时候先理清整个思路，把基本框架敲出来再细节，例：
    ...
    **分析**：
    ...
    - 循环（想到`for` `setTimeout` `setInterval`）,每隔一秒（说明定时，不能用`for`）`setTimeout`用调用自身的方法可实现`setInterval`功能，两种方法均列写  
    - ![image-20191130233012913](随笔（二）.assets/image-20191130233012913.png ":size=200") 基本思路如图所示，接下来就是`setInterval`实现
